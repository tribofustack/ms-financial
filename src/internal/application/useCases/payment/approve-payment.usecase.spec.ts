...
describe('ApprovePaymentByOrderId', () => {
  ...
  beforeAll(async () => {
    ...
    approvePaymentByOrderId = module.get<ApprovePaymentByOrderId>(ApprovePaymentByOrderId);
  });
  ...
  describe('execute', () => {
    ...
    it('should throw NotFoundException if the payment does not exist', async () => {
      ...
      await expect(approvePaymentByOrderId.execute(orderId)).rejects.toThrow(NotFoundException);
    });
  });
});
