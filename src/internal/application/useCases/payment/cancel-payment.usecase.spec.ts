...
describe('CancelPaymentByOrderId', () => {
  ...
  beforeAll(async () => {
    ...
    cancelPaymentByOrderId = module.get<CancelPaymentByOrderId>(CancelPaymentByOrderId);
  });
  ...
  describe('execute', () => {
    ...
    it('should throw DomainException if the payment was already approved', async () => {
      ...
      await expect(cancelPaymentByOrderId.execute(orderId)).rejects.toThrow(DomainException);
    });
    ...
    it('should throw NotFoundException if the payment does not exist', async () => {
      ...
      await expect(cancelPaymentByOrderId.execute(orderId)).rejects.toThrow(NotFoundException);
    });
  });
});
